...
│     │  │  │  │   │   │   │               │     │     │             │
│  ┌──▼──┼──┼──▼───▼┐ │   │  ┌────────┐ ┌─▼─────▼┐ ┌──▼──────────┐ │
│  │ Firebase       │ │   │  │Google  │ │Polygon │ │Google       │ │
│  │ Auth/Storage/  │ │   │  │Gemini  │ │RPC     │ │BigQuery     │ │
│  │ FCM            │ │   │  │(AI)    │ │        │ │(DSPデータ)  │ │
│  └────────────────┘ │   │  └────────┘ └────────┘ └─────────────┘ │
│                      │   │                                         │
│  ┌───────────────┐ ┌─▼───▼────────┐  ┌────────────────┐           │
│  │ Contentful    │ │ SendGrid     │  │ Stripe         │           │
│  │ (ニュースCMS) │ │ (メール送信)  │  │ (決済)         │           │
│  └──▲──▲─────────┘ └──────────────┘  └──▲──▲──────────┘           │
│     │  │                                 │  │                      │
└─────┼──┼─────────────────────────────────┼──┼──────────────────────┘
...
│  │ └──────────────────────┘          │                              │
│  │                                   │                              │
│  │ ※ Mobile → Backend APIはGraphQL経由│                              │
│  │ ※ Mobile Client → Firebase (認証/Storage/FCM)                    │
...
通信方向の凡例:
  ...
  WebUI Client → Polygon      : EVMアドレス検証
