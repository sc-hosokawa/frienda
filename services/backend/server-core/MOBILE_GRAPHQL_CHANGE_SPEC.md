...
| No | 内容 | 種類 | 主な対応 | ステータス |
| --- | --- | --- | --- | --- |
| 1 | API-39: `requestToAccessArtist` | 改修 | 申請メッセージを追加 | ステージング反映済み |
| 2 | API-44: `getBelongedArtists` | 改修 | 所属ステータスの追加・返却 | ステージング反映済み |
| 3 | アーティスト申請の再送用 Mutation | 新設 | 既存申請の再送・メッセージ更新 | ステージング反映済み |
| 4 | アーティスト申請のキャンセル用 Mutation | 新設 | 申請のキャンセル | ステージング反映済み |
| 5 | 所属先アーティストのデフォルト設定 Mutation | 新設 | デフォルト所属アーティスト設定 | ステージング反映済み |
| 6 | notifications のリスト取得 schema | 新設 | 管理者送付通知の一覧取得 | ステージング反映済み |
| 7 | notifications の個別コンテンツ取得 schema | 新設 | 管理者向け通知履歴・詳細取得 | ステージング反映済み |
| 8 | d-2 Query `getOverview` | 改修 | all / monthly / weekly / daily 合計を返却 | ToDo |
| 9 | Most Recent Release 用 Query | 新設 | リリース日が一番最近のリリース・楽曲を返却 | ToDo |
| 10 | Trending 用 Query | 新設 | ISRC単位で前期比の増加率上位3件を返却 | ToDo |
| 11 | d-4 Query `getTrending` | 改修 | all / monthly / weekly / daily 合計を返却 | ToDo |
| 12 | d-5 Query `getPlayCountHistory` | 改修 | 任意期間で絞り込み | ToDo |
| 13 | 国別再生数取得用 Query | 新設 | 国別再生数を返却 | ToDo |
| 14 | d-8 Query `getGenderGenRateByArtist` | 改修 | 世代区切り調整・不要 field 削除 | ToDo |
...
- モバイルアプリ側は、モバイルプッシュ channel で送付された通知を `getNotificationList` で取得・表示します。モバイル向けの通知詳細取得 Query は追加しません。
...
- `notifications`、`notification_channels`、送信先 snapshot、`notification_user` の作成は1つの DB transaction で実行します。途中で失敗した場合はすべて rollback します。
...
`deliveredUserCount` は、重複排除後に送信先 snapshot へ保存したユーザー数を返します。外部 push 通知や email の送信成功件数ではありません。
...
type AdminNotificationRecipient {
  ...
  username: String!
  email: String!
  mobilePushIsRead: Boolean
  mobilePushIsDeleted: Boolean
}
...
```graphql
query GetAdminNotificationList($limit: Int, $offset: Int) {
...
```graphql
query GetAdminNotificationDetail($notificationId: Int!) {
