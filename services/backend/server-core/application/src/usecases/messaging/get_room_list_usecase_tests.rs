...
use crate::usecases::messaging::get_room_list_usecase::{
...
async fn test_get_room_list_success() {
    ...
    let output = usecase
        ...
        .await
        .expect("room list should be returned for an existing user");
...
async fn test_get_active_rooms_success() {
    ...
    // Act
    let output = usecase
        .get_active_rooms(user_id.clone())
        .await
        .expect("active rooms should be returned for an existing user");
