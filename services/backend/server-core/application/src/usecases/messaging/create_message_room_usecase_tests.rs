use crate::test_support::mocks::{
    message_room_mock::MockMockRoomsRepo, user_room_mock::MockMockRoomUserRepo,
};
use crate::usecases::messaging::create_message_room_usecase::{
...
async fn test_create_new_message_room() {
    ...
    room_user_repo
        ...
        .returning(|room_users| {
            let user_ids: Vec<String> = room_users
                .iter()
                .map(|room_user| room_user.user_id.clone().unwrap())
                .collect();
            assert_eq!(
                user_ids,
                vec![
    ...
    let result = usecase
        ...
        .await
        .expect("new room creation should succeed");

    // Assert
    assert_eq!(result, room_id);
}
...
async fn test_return_existing_room_if_found() {
    ...
    let result = usecase
        ...
        .await
        .expect("existing room should be reused");

    // Assert
    assert_eq!(result, existing_room_id);
}
...
async fn test_create_room_with_duplicate_users() {
    ...
    let result = usecase
        ...
        .await
        .expect("room creation with duplicate users should succeed");
