...
async fn test_create_user_success() {
    ...
    mock_repo.expect_mock_create().times(1).returning({
        let expected_user = expected_user.clone();
        move |active_user| {
            assert_eq!(active_user.id.unwrap(), "user123");
            assert_eq!(active_user.fcm_token.unwrap(), Some("token123".to_string()));
            assert_eq!(active_user.email.unwrap(), "test@example.com");
            assert_eq!(active_user.username.unwrap(), "testuser");
            assert_eq!(active_user.realname.unwrap(), "Test User");
            assert_eq!(
                active_user.img_url.unwrap(),
                Some("http://example.com/image.png".to_string())
            );
            assert_eq!(
                active_user.invited_by.unwrap(),
                Some("inviter123".to_string())
            );
            assert_eq!(active_user.category.unwrap(), UserCategory::Creator);
            assert_eq!(active_user.primary_category.unwrap(), UserCategory::Creator);
