...
use crate::usecases::offer::search_tasks_usecase::{
...
async fn test_search_offers_by_category() {
    ...
    assert_eq!(offers[0].category, Some(OfferCategory::Creation));
}
...
async fn test_search_offers_by_price_range() {
    ...
    let offers = usecase
        ...
        .await
        .expect("price-range search should succeed");

    assert_eq!(offers.len(), 1);
    assert_eq!(offers[0].fee, 1000);
}
...
async fn test_search_offers_by_place() {
    ...
    let offers = usecase
        ...
        .await
        .expect("place-based search should succeed");

    assert_eq!(offers[0].place, "Tokyo".to_string());
}
...
async fn test_search_offers_by_target_role() {
    ...
    let offers = usecase
        ...
        .await
        .expect("role-based search should succeed");

    assert_eq!(offers[0].target_role, Some(UserCategory::Creator));
}
...
async fn test_search_offers_with_multiple_conditions() {
    ...
    let offers = usecase
        ...
        .await
        .expect("multi-condition search should succeed");

    assert_eq!(offers.len(), 1);
    assert_eq!(offers[0].place, "Tokyo");
}
...
async fn test_search_offers_with_invalid_price_range() {
    ...
    let offers = usecase
        ...
        .await
        .expect("repository should return an empty list for the mocked invalid range");
...
async fn test_search_offers_with_special_characters() {
    ...
    let offers = usecase
        ...
        .await
        .expect("special-character query should be forwarded to the repository");

    assert_eq!(offers.len(), 1);
    assert_eq!(offers[0].title, "Offer 1");
}
