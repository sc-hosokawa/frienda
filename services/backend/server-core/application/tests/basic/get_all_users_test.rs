...
use application::usecases::basic::get_all_users_usecase::{
    GetAllUsersUsecase, GetAllUsersUsecaseTrait,
};
...
async fn test_get_all_users_except_me_success() {
    ...
    // Act
    let result = usecase.get_all_users_except_me("1".to_string()).await;
