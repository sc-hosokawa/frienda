...
describe("Credential", function () {
  ...
  describe("Deployment", function () {
    it("Should set the right owner", async function () {
      ...
      expect(await credential.read.owner()).to.equal(
  ...
  describe("Minting", function () {
    it("Should allow owner to mint tokens", async function () {
      const { credential, owner, alice, bob } = await loadFixture(
        deployCredentialFixture
      );
      const amount = parseEther("100");
      await credential.write.mint(
        [alice.account.address, bob.account.address],
        [amount, amount]
      );
      expect(await credential.read.balanceOf(alice.account.address)).to.equal(
        amount
      );
      expect(await credential.read.balanceOf(bob.account.address)).to.equal(
        amount
      );
    });

    it("Should not allow non-owner to mint tokens", async function () {
      const { credential, otherAccount } = await loadFixture(
        deployCredentialFixture
      );
      const amount = parseEther("100");
      await expect(
        credential.write.mint([otherAccount.account.address, amount], {
  ...
  describe("Burning", function () {
    it("Should allow token holder to burn their tokens", async function () {
      const { credential, owner, otherAccount } = await loadFixture(
        deployCredentialFixture
      );
      const amount = parseEther("100");
      ...
      expect(
        await credential.read.balanceOf([otherAccount.account.address])
      ).to.equal(0n);
    });
  });
  ...
  describe("Transfer", function () {
    it("Should not allow transfer between accounts", async function () {
      ...
      await expect(
        ...
        "NonTransferableERC20: token transfers and approvals are disabled"
      );
    });
  });
});
